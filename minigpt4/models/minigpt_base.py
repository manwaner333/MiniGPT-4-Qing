...
class MiniGPTBase(BaseModel):
    ...
    #     return mixed_embs, seg_tokens

    def get_context_emb(self, prompt, img_list):
        device = img_list[0].device
        prompt_segs = re.split('<ImageHere>|<QuestoRes>', prompt)
        assert len(prompt_segs) == len(img_list) + 2, "Unmatched numbers of image placeholders and images."
        seg_tokens = [
            self.llama_tokenizer(
                seg, return_tensors="pt", add_special_tokens=i==0).to(device).input_ids # only add bos to the first seg
            for i, seg in enumerate(prompt_segs)
        ...
        mixed_embs = [emb for pair in zip(seg_embs[:-2], img_list) for emb in pair] + [seg_embs[-2]] + [seg_embs[-1]]
        mixed_embs = torch.cat(mixed_embs, dim=1)
        return mixed_embs, seg_tokens
