...
class Chat:
    ...
    def answer_prepare(self, conv, img_list, max_new_tokens=300, num_beams=1, min_length=1, top_p=0.9, top_k=5,
                       repetition_penalty=1.05, length_penalty=1, temperature=1.0, max_length=2000, output_hidden_states=True,
                       return_dict_in_generate=True, use_cache=True, output_scores=True, do_sample=False):
    ...
    #
    #     # combined_hidden_states = hidden_states[00][32][0].detach().cpu().numpy().tolist()
    #
    #     output_text = self.model.llama_tokenizer.batch_decode(output_token, skip_special_tokens=True)[0]
    #
    #     # output_text = output_text.split('###')[0]  # remove the stop sign '###'
    #     # output_text = output_text.split('Assistant:')[-1].strip()
    #     #
    #     # conv.messages[-1][1] = output_text
    #     return output_text, output_token.cpu().numpy(), combined_hidden_states
    ...
    def answer(self, conv, img_list, **kargs):
        ...
        # text
        output_token = outputs['sequences']
        output_text_ori = self.model.llama_tokenizer.batch_decode(output_token, skip_special_tokens=True)[0]
        output_text = output_text_ori.split('###')[0]
        output_text = output_text.split('Assistant:')[-1].strip()
        conv.messages[-1][1] = output_text
        ...
        #     top_logprob = {}
        #     for i in range(k):
        #         key = self.model.llama_tokenizer.decode(top_5_indices[i])
